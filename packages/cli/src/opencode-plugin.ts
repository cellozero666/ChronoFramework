 ...
 *
 * Tool policy version: TOOL_POLICY_VERSION (see @chrono/domain
...
import { OPENCODE_TOOL_POLICY, SKILL_RELEASE, TOOL_POLICY_VERSION, buildRuntimeFingerprint } from "@chrono/domain";
import { CHRONO_VERSION } from "./version.js";

const READ_TOOLS = ["glob", "grep", "lsp", "question", "read", "skill", "todowrite"];

/**
 * Canonical policy slice for one gate classification, sorted for
 * deterministic generated bytes. Single source of truth: the
 * generated gate sets below derive from this, never from a copy.
 */
function policyTools(classification: string): string[] {
  return Object.keys(OPENCODE_TOOL_POLICY)
    .filter((name) => OPENCODE_TOOL_POLICY[name] === classification)
    .sort();
}
const MUTATE_TOOLS = [
  "apply_patch",
...
// Native governed tool sets (OC-P11 correction, C5; CORE_FIX
// vertical; planning runway): DERIVED from the canonical domain
// policy (OPENCODE_TOOL_POLICY) at generation time — never
// hardcoded here. A tool added to the runtime surface without a
// policy entry cannot reach the generated gate, and any divergence
// fails the policy-surface parity test. The gate requires proven
// entry, nothing more; each tool enforces its own session and the
// Core validates every call.
const PLANNING_TOOLS = new Set(${JSON.stringify(policyTools("planning"))});
// Native governed lifecycle tools (CORE_FIX vertical + planning
// runway): narrow, capability-gated state operations — derived from
// the canonical policy like PLANNING_TOOLS above. Unbound
// (Gaspar/operator) sessions may call them — each enforces its own
// session and the Core validates every call. Bound worker sessions
// may call them too (evidence, status, release, reviews); the Core
// enforces binding, role, and scope per call. Planning tools stay
// denied to workers.
const LIFECYCLE_TOOLS = new Set(${JSON.stringify(policyTools("lifecycle"))});
...
export const ChronoGatePlugin = async (ctx) => {
  ...
  try {
    ...
    if (loadProject !== null && !pluginLoadLogged) {
      ...
      logEvidence(loadProject, {
        kind: "plugin-load",
        // Runtime generation handshake (stale-loaded-plugin repair):
        // every load records the exact generation it runs — CHRONO
        // version, tool policy, deterministic build fingerprint, and
        // a per-load process identifier with timestamp (added by the
        // evidence writer). Doctor compares the latest load against
        // the installed expectations and blocks on mismatch until a
        // full OpenCode restart loads a matching generation.
        chronoVersion: "${CHRONO_VERSION}",
        toolPolicyVersion: "${TOOL_POLICY_VERSION}",
        buildFingerprint: ${JSON.stringify(buildRuntimeFingerprint())},
        loadId: process.pid + "-" + Math.random().toString(16).slice(2, 10),
      });
    }
  } catch {
  ...
  return {
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (binding !== null) {
        ...
        if (kind !== "mutate") {
          throw new Error(
            \`[chrono] TOOL_DENIED: tool '\${tool}' is not classified by CHRONO tool policy v${TOOL_POLICY_VERSION}: deny-by-default until reviewed.\`
          );
        }
