  ...
  describe("Packed OpenCode planning surface (OC-P11 req 5)", () => {
    ...
    it("packed install exposes init dry-run and the artifact surface", async () => {
      ...
      try {
        ...
        // runtime (exact generated bytes install through setup). No
        // approval-confirm tool may ship: signing follows only an
        // explicit human answer observed by the plugin host.
        const cliDist = join(install, "node_modules", "@chrono", "cli", "dist");
        const { readFileSync: readPacked } = await import("node:fs");
        const generatedTools = readPacked(join(cliDist, "opencode-planning-tools.js"), "utf8");
        expect(generatedTools).toContain("chrono_artifact_supersede");
        expect(generatedTools).not.toContain("chrono_approval_confirm");
